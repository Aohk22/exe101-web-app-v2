...
async function seed() {
  ...
  const insertedLessons = await db.insert(lessons).values(allLessons).returning()
  ...
  await db.insert(usersToCourses).values([
    ...
    { userId: insertedUsers[1].id, courseId: courseById['1'].id, progress: 100, completed: true },
    { userId: insertedUsers[1].id, courseId: courseById['4'].id, progress: 60,  completed: false },
  ])
