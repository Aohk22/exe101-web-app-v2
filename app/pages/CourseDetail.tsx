      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <Link to="/courses" className="text-sm text-slate-500 hover:text-emerald-400 transition-colors">Courses</Link>
            <ChevronRight className="w-4 h-4 text-slate-700" />
            <span className="text-sm text-slate-200 font-medium">{course.title}</span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-6 py-4 border-y border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-white">15,420 enrolled</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-slate-500">{MOCK_MODULES.length} modules • {course.lessonsCount} lessons</p>
          </div>

          <div className="space-y-4">
            {MOCK_MODULES.map((module, i) => (
              <div key={module.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                <div className="p-4 bg-slate-800/50 border-b border-slate-800 flex items-center justify-between">
                  <h3 className="font-bold text-white">Module {i + 1}: {module.title}</h3>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          ...
                          lesson.completed ? "bg-emerald-500/10 text-emerald-500" : "bg-slate-800 text-slate-600 group-hover:bg-emerald-500/10 group-hover:text-emerald-400"
                        )}>
                        ...
                        <div>
                          <p className={cn(
                            ...
                            lesson.completed ? "text-slate-500" : "text-slate-200 group-hover:text-emerald-400"
                          )}>
                            {lesson.title}
                          </p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-[10px] font-bold uppercase text-slate-500">{lesson.type}</span>
