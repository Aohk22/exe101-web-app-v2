    ...
    <div className="flex flex-col h-[calc(100vh-120px)] text-slate-200">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">3. Network Reconnaissance & Scanning</h1>
            ...
            <div className="prose prose-invert max-w-none">
              {activeTab === 'overview' && (
                <div className="space-y-4">
                  <h2 className="text-xl font-bold text-white">About this lesson</h2>
                  <p className="text-slate-300 leading-relaxed">
                    In this lesson, we'll dive deep into network reconnaissance techniques. We'll explore how to identify active hosts, open ports, and running services on a target network. By the end of this module, you'll understand:
                  </p>
                  <ul className="list-disc list-inside space-y-2 text-slate-300">
                    <li>Passive vs Active Reconnaissance</li>
                    <li>Nmap scanning techniques and flags</li>
                    <li>Banner grabbing and service enumeration</li>
                    <li>Best practices for stealthy scanning</li>
                  </ul>
                </div>
              )}
              {activeTab === 'resources' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    { name: 'Nmap_Cheat_Sheet.pdf', size: '1.2 MB' },
                    { name: 'Lab_Network_Topology.png', size: '0.8 MB' },
                    { name: 'Recon_Tools_Guide.epub', size: '2.4 MB' },
                  ].map((file, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-2xl hover:border-emerald-500/50 transition-colors group cursor-pointer">
                      ...
                      <button className="text-emerald-400 text-xs font-bold">Download</button>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="lg:col-span-1 flex flex-col bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-sm min-h-0">
          <div className="p-5 border-b border-slate-800 bg-slate-800/50">
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-xs text-slate-400">12/24 lessons completed</p>
            ...
            {[1, 2, 3].map((module) => (
              <div key={module}>
                <div className="px-5 py-3 bg-slate-800/50 border-y border-slate-800">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Module {module}</p>
                </div>
                  ...
                  {[1, 2, 3, 4].map((lesson) => (
                    <div 
                      ...
                      className={`p-4 flex items-center gap-3 hover:bg-slate-800 transition-colors cursor-pointer group ${
                        module === 1 && lesson === 3 ? 'bg-emerald-500/10' : ''
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs font-medium truncate ${
                          module === 1 && lesson === 3 ? 'text-emerald-400' : 'text-slate-300'
                        }`}>
                          {lesson === 1 ? 'Intro to Cyber Threats' : 
                           lesson === 2 ? 'Setting up Kali Linux' :
                           lesson === 3 ? 'Network Recon & Scanning' :
                           'Practical Lab: Nmap'}
                        </p>
