...
import { User, Trophy, CreditCard, History, Camera, Star, Flame, Award, Zap, ChevronRight, Clock, BookOpen } from 'lucide-react';
...
export default function Profile() {
  ...
  const transactions = [
    { id: 'TX-12345', date: 'Mar 10, 2026', amount: '$49.00', status: 'Completed', item: 'Pro Monthly Subscription' },
    { id: 'TX-12344', date: 'Feb 10, 2026', amount: '$49.00', status: 'Completed', item: 'Pro Monthly Subscription' },
    { id: 'TX-12343', date: 'Jan 15, 2026', amount: '$19.00', status: 'Completed', item: 'Advanced Network Security Course' },
  ];

  const learningHistory = [
    { id: 1, course: 'Network Defense & Hardening', lesson: 'Module 3: Firewalls', date: 'Today, 10:30 AM', duration: '45m' },
    { id: 2, course: 'Ethical Hacking Essentials', lesson: 'Final Exam', date: 'Yesterday', duration: '2h 15m' },
    { id: 3, course: 'Cloud Security Fundamentals', lesson: 'Module 1: Intro to AWS', date: 'Mar 12, 2026', duration: '30m' },
  ];
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      <header className="mb-10">
        ...
        <p className="text-slate-400 mt-1">Manage your personal information and track your journey.</p>
      </header>
      ...
      <div className="space-y-6">
        ...
        >
            {activeTab === 'profile' && (
              <div className="space-y-8">
                ...
                <div className="pt-6 border-t border-slate-800 flex justify-end gap-4">
                  <button className="px-8 py-3 bg-emerald-600 text-white rounded-2xl text-sm font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-900/20 transition-all">Save Changes</button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-sm font-bold text-emerald-400">
                    <Award className="w-5 h-5" /> 3 Certificates Earned
                  </div>
                </div>

                <div className="grid grid-cols-1 gap-4">
                  {[
                    { title: 'Ethical Hacking Professional', date: 'Mar 12, 2026', issuer: 'CyberSpace Academy', id: 'CERT-EH-9921' },
                    { title: 'Network Defense Specialist', date: 'Feb 28, 2026', issuer: 'CyberSpace Academy', id: 'CERT-ND-8812' },
                    { title: 'Cloud Security Fundamentals', date: 'Jan 15, 2026', issuer: 'CyberSpace Academy', id: 'CERT-CS-7734' },
                  ].map((cert) => (
                    <div key={cert.id} className="p-6 bg-slate-800/50 border border-slate-800 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-emerald-500/50 transition-all group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">Issued by {cert.issuer} • {cert.date}</p>
                          <p className="text-[10px] font-mono text-slate-500 mt-1">ID: {cert.id}</p>
                        </div>
                      </div>
                ...
                <div className="pt-6 border-t border-slate-800">
                  <div className="bg-emerald-500/10 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="text-center md:text-left">
                      <h3 className="font-bold text-emerald-400 mb-1">Earn more certificates</h3>
                      <p className="text-sm text-emerald-300/70">Complete courses and pass final exams to validate your skills.</p>
                    </div>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-slate-800">
                        <th className="pb-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Item</th>
                        <th className="pb-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Date</th>
            ...
            {activeTab === 'learning' && (
              <div className="space-y-8">
                <h2 className="text-xl font-bold text-white mb-6">Learning History</h2>
                <div className="space-y-4">
                  {learningHistory.map((log) => (
                    <div key={log.id} className="p-4 bg-slate-800/50 border border-slate-800 rounded-2xl flex items-center justify-between hover:border-emerald-500/30 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center shadow-sm">
                          <BookOpen className="w-5 h-5 text-emerald-500" />
                        </div>
                        <div>
                          <h4 className="font-bold text-white text-sm">{log.course}</h4>
                          <p className="text-xs text-slate-400">{log.lesson}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-bold text-white">{log.date}</p>
                ...
                <button className="w-full py-3 bg-slate-800 text-slate-400 rounded-2xl text-sm font-bold hover:bg-slate-700 hover:text-white transition-colors flex items-center justify-center gap-2">
                  Download Learning Report <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            )}
          </motion.div>
